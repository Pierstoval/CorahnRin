...
class DetailsProcessor implements LegacyCharacterConversionProcessor
{
    public function process(LegacyCharacterDTO $characterToUpdate, LegacyCharacterData $legacyData): void
    {
        $characterToUpdate->setSex($legacyData->getSex());
        $characterToUpdate->setDescription($legacyData->getDescription());
        $characterToUpdate->setAge($legacyData->getAge());
    }
}
