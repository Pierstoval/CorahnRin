...
class Step05SocialClassTest extends AbstractStepTest
{
    ...
    public function testInvalidSocialClass(): void
    {
        ...
        static::assertEquals('Veuillez sélectionner une classe sociale valide.', $crawler->filter('#flash-messages > .card-panel.error')->text('', true));
    }
    ...
    public function testValidSocialClassButNotAssociatedDomains(): void
    {
        ...
        static::assertSame(200, $result->getResponse()->getStatusCode());
        static::assertCount(1, $crawler->filter('#flash-messages > .card-panel.error'));
        static::assertEquals('Les domaines choisis ne sont pas associés à la classe sociale sélectionnée.', $crawler->filter('#flash-messages > .card-panel.error')->text('', true));
    }
    ...
    public function testValidSocialClassButNotEnoughDomains(): void
    {
        ...
        static::assertEquals('Vous devez choisir 2 domaines pour lesquels vous obtiendrez un bonus de +1. Ces domaines doivent être choisi dans la classe sociale sélectionnée.', $crawler->filter('#flash-messages > .card-panel.warning')->text('', true));
    }
}
